...
# Shallow RVFL Function
def shallow_rvfl(X_train, Y_train, L, C, X_test, Y_test):
...
for L in L_values:
    for C in C_values:
        acc = shallow_rvfl(X_train, Y_train, L, C, X_val, Y_val)
        if acc > best_acc:
            best_acc = acc
            best_L, best_C = L, C

# Final evaluation on the test set
final_acc = shallow_rvfl(X_train_full, Y_train_full, best_L, best_C, X_test, Y_test)
